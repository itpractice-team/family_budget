...
class TestDockerfileCompose:
    ...
    def test_docker_compose_file(self):
        ...
        assert re.search(r"image:\s+postgres:", docker_compose), (
            "Проверьте, что  в файл docker-compose.yaml " "добавлен образ postgres"
        )
        assert re.search(
            r"image:\s+([a-zA-Z0-9]+)\/([a-zA-Z0-9_\.])+(\:[a-zA-Z0-9_-]+)?",
            docker_compose,
        ), (
            "Проверьте, что добавили сборку контейнера из образа "
            "на вашем DockerHub в файл docker-compose.yaml"
        )
